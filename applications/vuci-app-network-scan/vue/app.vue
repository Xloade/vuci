<template>
  <div class="example">
    <div class="overlayedArea">
      <Overlay v-if="inProgress">
        <Progress :progress="progress"/>
      </Overlay>
      <Overlay v-else-if="!subnets.length"/>
...
export default {
  ...
  methods: {
    ...
    getPosition (index, itemCount, offsetY) {
      ...
      const startLeft = (this.neededWidth / 2 - groupWidth / 2) + this.itemsMargin / 2 + 100
      const itemPlace = ((groupWidth - this.itemsMargin) / (itemCount) * index) + startLeft
    ...
    getHostNumFromSubNet (subnet) {
      ...
      return (subnet.hosts.length === null || subnet.hosts.length === undefined) ? 0 : subnet.hosts.length
    },
    getOffsetY (index) {
      return index === 0 ? 300 : -300
    },
    widthFromItemCount (count) {
      return count * 200 + this.itemsMargin
    }
  },
  computed: {
    neededWidth () {
